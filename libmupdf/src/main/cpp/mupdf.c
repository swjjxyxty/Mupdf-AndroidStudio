    ...
    fz_try(ctx)
                        {
                            ...
                            fz_try(ctx)
                                                {
                                                    glo->current_path = fz_strdup(ctx,
                                                                                  (char *) filename);
                                                    glo->doc = fz_open_document(ctx,
                                                                                (char *) filename);
                                                    alerts_init(glo);
                                                }
                            fz_catch(ctx) {
                            ...
                            fz_try(ctx)
                                                {
                                                    ...
                                                    glo->doc = fz_open_document_with_stream(ctx,
                                                                                            magic,
                                                                                            stream);
                                                    alerts_init(glo);
                                                }
...
static void update_changed_rects(globals *glo, page_cache *pc, pdf_document *idoc) {
    ...
    while ((annot = (fz_annot *) pdf_poll_changed_annot(ctx, idoc, (pdf_page *) pc->page)) !=
           NULL) {
    ...
    fz_try(ctx)
                        {
                            ...
                            if (pc->annot_list == NULL) {
                                ...
                                for (annot = fz_first_annot(ctx,
                                                            pc->page); annot; annot = fz_next_annot(
                                        ctx, pc->page, annot))
                            ...
                            pix = fz_new_pixmap_with_bbox_and_data(ctx, glo->colorspace, &pixbbox,
                                                                   pixels);
...
JNI_FN(MuPDFCore_updatePageInternal)(JNIEnv *env, jobject thiz, jobject bitmap, int page,
                                     int pageW, int pageH, int patchX, int patchY, int patchW,
                                     int patchH, jlong cookiePtr) {
    AndroidBitmapInfo info;
    void *pixels;
    int ret;
    fz_device *dev = NULL;
    ...
    fz_try(ctx)
                        {
                            ...
                            for (crect = hq ? pc->hq_changed_rects
                                            : pc->changed_rects; crect; crect = crect->next) {
                                fz_irect abox;
                                ...
                                if (!fz_is_empty_irect(&abox)) {
                                    ...
                                    if (pc->page_list)
                                        fz_run_display_list(ctx, pc->page_list, dev, &ctm, &arect,
                                                            cookie);
                                    if (cookie != NULL && cookie->abort)
                                        fz_throw(ctx, FZ_ERROR_GENERIC, "Render aborted");

                                    if (pc->annot_list)
                                        fz_run_display_list(ctx, pc->annot_list, dev, &ctm, &arect,
                                                            cookie);
                                    if (cookie != NULL && cookie->abort)
                                        fz_throw(ctx, FZ_ERROR_GENERIC, "Render aborted");
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addMarkupAnnotationInternal)(JNIEnv *env, jobject thiz, jobjectArray points,
                                              fz_annot_type type) {
    globals *glo = get_globals(env, thiz);
    fz_context *ctx = glo->ctx;
    fz_document *doc = glo->doc;
    ...
    fz_try(ctx)
                        {
                            ...
                            annot = (fz_annot *) pdf_create_annot(ctx, idoc, (pdf_page *) pc->page,
                                                                  type);

                            pdf_set_markup_annot_quadpoints(ctx, idoc, (pdf_annot *) annot, pts, n);
                            pdf_set_markup_appearance(ctx, idoc, (pdf_annot *) annot, color, alpha,
    ...
    fz_try(ctx)
                        {
                            ...
                            pdf_set_ink_annot_list(ctx, idoc, (pdf_annot *) annot, pts, counts, n,
                                                   color, INK_THICKNESS);
    ...
    fz_try(ctx)
                        {
                            ...
                            if (annot) {
                                pdf_delete_annot(ctx, idoc, (pdf_page *) pc->page,
                                                 (pdf_annot *) annot);
                                dump_annotation_display_lists(glo);
                            }
                        }
...
JNI_FN(MuPDFCore_getWidgetAreasInternal)(JNIEnv *env, jobject thiz, int pageNumber) {
    ...
    count = 0;
    for (widget = pdf_first_widget(ctx, idoc,
                                   (pdf_page *) pc->page); widget; widget = pdf_next_widget(ctx,
                                                                                            widget))
        count++;
...
JNI_FN(MuPDFCore_getAnnotationsInternal)(JNIEnv *env, jobject thiz, int pageNumber) {
    ...
    count = 0;
    for (annot = fz_first_annot(ctx, pc->page); annot; annot = fz_next_annot(ctx, pc->page,
                                                                             annot)) {
        fz_rect rect;
...
JNIEXPORT int JNICALL
JNI_FN(MuPDFCore_passClickEventInternal)(JNIEnv *env, jobject thiz, int pageNumber, float x,
                                         float y) {
    globals *glo = get_globals(env, thiz);
    fz_context *ctx = glo->ctx;
    fz_matrix ctm;
    ...
    fz_try(ctx)
                        {
                            ...
                            if (idoc) {
                                ...
                                if (focus) {
                                    result = pdf_text_widget_set_text(ctx, idoc, focus,
                                                                      (char *) text);
                                    dump_annotation_display_lists(glo);
                                }
                            }
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_setFocusedWidgetChoiceSelectedInternal)(JNIEnv *env, jobject thiz,
                                                         jobjectArray arr) {
    globals *glo = get_globals(env, thiz);
    fz_context *ctx = glo->ctx;
    pdf_document *idoc = pdf_specifics(ctx, glo->doc);
...
JNI_FN(MuPDFCore_getFocusedWidgetSignatureState)(JNIEnv *env, jobject thiz) {
    ...
    return pdf_dict_get(ctx, ((pdf_annot *) focus)->obj, PDF_NAME_V) ? Signature_Signed
                                                                     : Signature_Unsigned;
}
...
JNIEXPORT jboolean JNICALL
JNI_FN(MuPDFCore_signFocusedSignatureInternal)(JNIEnv *env, jobject thiz, jstring jkeyfile,
                                               jstring jpassword) {
    globals *glo = get_globals(env, thiz);
    fz_context *ctx = glo->ctx;
    pdf_document *idoc = pdf_specifics(ctx, glo->doc);
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_controlSepOnPageInternal)(JNIEnv *env, jobject thiz, int page, int sep,
                                           jboolean disable) {
    globals *glo = get_globals(env, thiz);
    fz_context *ctx = glo->ctx;
    int i;
...
JNI_FN(MuPDFCore_getSepInternal)(JNIEnv *env, jobject thiz, int page, int sep) {
    ...
    name = fz_get_separation_on_page(ctx, glo->pages[i].page, sep, (unsigned int *) (&rgba[0]),
                                     &cmyk);
